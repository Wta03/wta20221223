...
<template>
    <div id="di">
        <br>
        <el-page-header @back="goBack" content="用户信息管理">
        </el-page-header>
        <br>
        ...
        <el-drawer title="信息修改" :visible.sync="drawer" :direction="direction" :before-close="handleClose" style="padding:50px;margin:50px">
        <div style="padding:50px">
            <span> 用户名:</span>
            <br>
        ...
        <div style="margin: 20px 0px">
            <el-button @click="tianjia()" type="primary">添加用户</el-button>
            <el-button @click="toggleSelection()">取消选择</el-button>
        </div>
        <el-table ref="multipleTable" tooltip-effect="dark" style="width: 100%" stripe
            @selection-change="handleSelectionChange" border :data="pas.filter(data => !search || data.user_name.toLowerCase().includes(search.toLowerCase()))">
...
export default {
    ...
    methods: {
        update1(){
            ...
            axios.post('http://localhost:3000/api/Stu/updateStu1', { user_password: this.us.user_password, user_name:this.us.user_name,user_pav:'普通用户'}).then((response) => {
                console.log(response);
            })
        },
        update2() {
        ...
        showStu1() {
            axios.post('http://localhost:3000/api/Stu/showStu_alluser', {}).then((response) => {
